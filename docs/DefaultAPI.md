...
------------- | ------------- | -------------
[**GetDescription**](DefaultAPI.md#GetDescription) | **Get** /api/v2/product/description | Get Description
...
> DescriptionParseResponse GetDescription(ctx).ProductId(productId).Country(country).Currency(currency).Locale(locale).Execute()
...
func main() {
	...
	resp, r, err := apiClient.DefaultAPI.GetDescription(context.Background()).ProductId(productId).Country(country).Currency(currency).Locale(locale).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DefaultAPI.GetDescription``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetDescription`: DescriptionParseResponse
...
[**DescriptionParseResponse**](DescriptionParseResponse.md)
...
func main() {
	...
	// response from `GetShippingInfo`: ShippingParseResponse
	fmt.Fprintf(os.Stdout, "Response from `DefaultAPI.GetShippingInfo`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetShippingInfoRequest struct via the builder pattern
